...
def handle_tile_placed(ctx, item):
    ...
    Info.debug("EVENT_DEBUG", f"{tile_type} tile PLACED at {tile_id}")
    Info.debug("NEW_TILE_UHI_DEBUG", f"UHI of new tile: {uhi_array[ctx.tile_manager.get_tile_position(tile_id)[0], ctx.tile_manager.get_tile_position(tile_id)[1]]}")
...
def handle_tile_removed(ctx, item):
    ...
    Info.debug("EVENT_DEBUG", f"{tile_type} tile REMOVED at {tile_id}")
    Info.debug("NEW_TILE_UHI_DEBUG",
